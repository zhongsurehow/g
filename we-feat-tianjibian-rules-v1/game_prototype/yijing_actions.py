...
def enhanced_play_card(game_state: GameState, card_index: int, target_gua: str, mods=None) -> GameState:
    ...
    if balance_bonus > 0:
        ...
        print(f"☯️ 阴阳平衡奖励：+{balance_bonus}气")
...
def divine_fortune(game_state: GameState) -> GameState:
    ...
    if current_player.qi < 2:
        print(f"❌ {current_player.name} 气不足，无法进行占卜（需要2点气）")
        return new_state
    ...
    print(f"📜 得卦：{divination['gua']}")
    print(f"🌟 运势：{divination['fortune']}")
    print(f"💡 建议：{divination['advice']}")
...
def consult_yijing(game_state: GameState, action_type: str) -> GameState:
    ...
    if current_player.dao_xing < 2:
        print(f"❌ {current_player.name} 道行不足，无法咨询易经（需要2点道行）")
        return new_state
...
def enhanced_study(game_state: GameState, mods=None) -> GameState:
    ...
    if current_player.wuxing_affinities[dominant_element] >= 5:
        cards_to_draw += 1
        print(f"📚 {dominant_element.value}行精通：额外抽取1张卡牌")
...
def wuxing_interaction(game_state: GameState, element1: WuXing, element2: WuXing) -> GameState:
    ...
    if WuXingCycle.is_sheng_relationship(element1, element2):
        ...
        print(f"⚔️ 五行相克：{element1.value}克{element2.value}，获得制衡之道")
...
def display_yijing_status(player: Player):
    """显示玩家的易经修行状态"""
    print(f"\n📊 {player.name} 的修行状态:")
    print(f"   ☯️  阴阳平衡: 阴{player.yin_yang_balance.yin_points} 阳{player.yin_yang_balance.yang_points} (平衡度: {player.yin_yang_balance.balance_ratio:.2f})")
    ...
    if player.active_wisdom:
        print(f"   💡 激活智慧: {', '.join(player.active_wisdom)}")
